...
- **Listing icon caveat**: the AMO listing icon showed the default placeholder since the listing was created (the first listed versions used SVG icons, which AMO can't use for the listing image). There is no manual icon upload on the main Edit page — the icon upload lives at `Edit → Images Edit → Edit` (`/edit_media/edit`, `input[name="icon_upload"]`). **Two traps found 2026-09-25**: (1) the `icon_type` radio (default/jpeg/png) must be set to `image/png` — without it the view silently ignores the uploaded file and re-renders with no error; (2) the POST must include the screenshots formset management fields — submit via `new FormData(form)` (includes everything), never a hand-rolled FormData. After a successful save the API `icons` field points to `user-media/addon_icons/<id>/...` — if those URLs 404 while other add-ons' icons load (checked: uBlock's works), the media file write failed server-side: escalate via the Add-ons Discourse forum (addons.mozilla.org subcategory) or addons-support; re-uploading may retry the media write.
